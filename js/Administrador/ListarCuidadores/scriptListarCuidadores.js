$(document).ready(function () {
    ...
    var botao = 0;
    $.post("http://3.96.217.5/lib/libListarCuidadoresAdm.aspx", { control: botao }, function (retorno) {
        if (retorno == "false") {
            console.log("deu erro na lib");
        }
        else {
            if (retorno == "") {
                $('.listaCuidadores').html("<h2 style='font-family: Rubik;text-align:center;margin:60px auto;width:80%;color:white;'>Desculpe, não temos nenhum cuidador com essas especificações/h2>");
            }
            else {
    ...
    $(document).on("click", "#buscar_filtro", function () {
        ...
        $.post("http://3.96.217.5/lib/libListarCuidadoresAdm.aspx", { vE: vE, vS: vS, vP: vP, vA: vA, vEm: vEm, vG: vG, especializacao: filtroEspecializacao, status: filtroStatus, preco: filtroPreco, avaliacao: filtroAvaliacao, emailCuidador: filtroEmail, genero: filtroGenero, control: botao }, function (retorno) {
            if (retorno == "false") {
                console.log("deu erro na lib");
            }
            else 
