...
{
    public class clsPaciente: clsBanco_32623
    {
        ...
        public string nm_cuidador = "";
        public List<string> cd_servico { get; set; }
        public string dt_inicio_servico { get; set; }
        public string dt_semana { get; set; }
        public string hr_inicio_servico { get; set; }
        public string hr_fim_servico { get; set; }
        ...
        public bool buscarPacientes(string emailUsuario) 
        {
            ...
            if (!Procedure("buscarPacientes", true, valores, ref dados))
            {
                Desconectar();
                return false;
            }
            ...
            if (dados.HasRows)
            {
                while (dados.Read())
                {
                    cd_paciente.Add(dados[0].ToString());
                    nm_paciente.Add(dados[1].ToString());
                    nm_cidade.Add(dados[2].ToString());
                    nm_estado.Add(dados[3].ToString());
                    if (!Convert.IsDBNull(dados[4]))
                    {
                        byte[] imagem = (byte[])dados[4];
        ...
        #region Endereço 
        public bool confirmarEndereco(string codigoPaciente)
        {
            MySqlDataReader dados = null;
            string[,] valores = new string[1, 2];
            ...
            if (!Procedure("buscarEnderecoPaciente", true, valores, ref dados))
            {
                Desconectar();
                return false;
            }
            ...
            if (dados.HasRows)
            {
                while (dados.Read())
                {
                    nm_cidade.Add(dados[0].ToString());
                    nm_rua = dados[1].ToString();
        ...
        #region Buscar Dados dos Pacientes
        public bool buscarDadosPaciente(string cd_paciente)
        {
            MySqlDataReader dados = null;
            string[,] valores = new string[1, 2];
            valores[0, 0] = "vCdPaciente";
            valores[0, 1] = cd_paciente;
            base64standard = "";

            if (!Procedure("buscarDadosPaciente", true, valores, ref dados))
            {
                Desconectar();
                return false;
            }
            ...
            if (dados.HasRows)
            {
                while (dados.Read())
                {
                    ...
                    nm_tipo_necessidade_paciente = (dados[1].ToString());
                    ds_paciente = (dados[2].ToString());
                    cep = (dados[3].ToString());
                    nm_cidade.Add(dados[4].ToString());
                    nm_bairro = (dados[5].ToString());
                    nm_rua = (dados[6].ToString());
                    nm_num = (dados[7].ToString());
                    nm_estado.Add((dados[8].ToString()));
                    nm_complemento = (dados[9].ToString());
        ...
        #region Buscar Pacientes em Serviço
        public bool buscarPacientesEmServico(string emailUsuario)
        {
            MySqlDataReader dados = null;
            string[,] valores = new string[1, 2];
            valores[0, 0] = "vEmailUsuario";
            ...
            if (!Procedure("buscarPacienteServicoEmAndamento", true, valores, ref dados))
            {
                Desconectar();
                return false;
            }
            ...
            if (dados.HasRows)
            {
                while (dados.Read())
                {
                    ...
                    nm_cidade.Add(dados[2].ToString());
                    nm_estado.Add(dados[3].ToString());
                    cd_servico.Add(dados[4].ToString());
                }
        ...
        #region Informação Serviço Atual
        public bool infoServicoAtual(string cdServico)
        {
            ...
            if (!Procedure("infoServicoAtual", true, valores, ref dados))
            {
                Desconectar();
                return false;
            }
            ...
            if (dados.HasRows)
            {
                while (dados.Read())
                {
                    ...
                    nm_cuidador = dados[1].ToString();
                    nm_rua = dados[2].ToString();
                    nm_num = dados[3].ToString();
                    nm_bairro = dados[4].ToString();
                    dt_inicio_servico = dados[5].ToString();
                    dt_semana = dados[6].ToString();
                    hr_inicio_servico = dados[7].ToString();
                    hr_fim_servico = dados[8].ToString();
